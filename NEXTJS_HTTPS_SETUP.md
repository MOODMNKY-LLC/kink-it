...
- **Supabase TLS**: ✅ Correctly enabled in `supabase/config.toml` (`[api.tls] enabled = true`)
...
- `pnpm run dev` → HTTPS server (uses `server.js`)
- `pnpm run dev:https` → HTTPS server (explicit)
- `pnpm run dev:http` → Standard HTTP server (fallback)

## 🔄 Supabase vs Next.js HTTPS

**Important Distinction:**

| Service | Configuration | Certificates |
|---------|--------------|--------------|
| **Supabase** | `supabase/config.toml` → `[api.tls] enabled = true` | Auto-generated by Supabase |
...
**What You Did Right:**
- ✅ Enabled TLS in Supabase `config.toml` (correct!)
- ✅ Supabase auto-generates certificates (no manual creation needed)
